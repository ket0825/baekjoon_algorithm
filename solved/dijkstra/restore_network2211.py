...
2 초	192 MB	8238	4249	3019	50.527%
문제
...
def dijkstra_path(graph, N):
    edges = {} # next node: current node
    pq = [(0, 1)] # weight, start
    ...
    while pq:
        ...
        # print(f"pq: {pq}")
        if distances[cur_node] < cur_weight:
            continue
            
        for next_node, next_weight in graph[cur_node]:
            if distances[next_node] > cur_weight + next_weight:
                distances[next_node] = cur_weight + next_weight                
                edges[next_node] = cur_node
                # print(f"edges: {edges}")
                heapq.heappush(pq, (cur_weight + next_weight, next_node))
...
if __name__ == '__main__':
    ...
    answer = dijkstra_path(graph, N)
    print(len(answer))
    for s, e in answer.items():
        print(s, e)
        

"""
출처: seroak
일반적으로 path, parent 등의 변수를 둬서 여기에 자취를 기입함.
