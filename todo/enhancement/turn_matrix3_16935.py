...
for cmd in cmds:
   match cmd:
      case 1:
         # vertical reverse. N/2 * M (for문 2번)보다 list 자체들을 역으로 만드는게 더욱 빠름.
         mat = mat[::-1]
      case 2:
         # horizontal reverse. N * M/2 (for문 2번)보다 내부 list들을 역으로 만드는게 더욱 빠름.
         mat = [row[::-1] for row in mat]      
      case 3:
         temp_mat = [[0]*N for _ in range(M)] # [0]*N와 단순 list comprehension: 약 40배 차이
         for col in range(M):
               for row in range(N):           
                  temp_mat[col][N-1-row] = mat[row][col]
         M, N = N, M
         mat = temp_mat
      ...
      case 4:
         ...
         for row in range(N):
            for col in range(M):
                  temp_mat[M-1-col][row] = mat[row][col]
         M, N = N, M
         mat = temp_mat
      ...
      case 5:
         for row in range(N//2):
             for col in range(M//2):
               mat[row][col+M//2], mat[row+N//2][col+M//2], mat[row+N//2][col], mat[row][col]\
                  = mat[row][col], mat[row][col+M//2], mat[row+N//2][col+M//2], mat[row+N//2][col]
...
for row in mat: # for문 2번보다 2배 for문 1번 + unpacking이 2배 빠름.
   print(*row)
